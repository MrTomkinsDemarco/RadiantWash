...
class LocationController: UIViewController {
  ...
  private var locationGridController = LocationGridController()
...
extension LocationController {
  ...
  private func setupShowLocationInfoController(segue: UIStoryboardSegue, sender: Any?) {
    ...
    if let locationInfoController = segue.destination as? LocationInfoController {
      if let phasset = sender as? PHAsset {
        locationInfoController.currentPhasset = phasset
        if let date = phasset.creationDate {
          locationInfoController.title = Utils.getString(from: date, format: Constants.dateFormat.readableFormat)
        } else {
          locationInfoController.title = Localization.Main.Title.location
        }
        
        locationInfoController.removeSelectedPHAsset = { phasset in
...
extension LocationController: LocationGridDelegate {
...
extension LocationController: AnnotationViewSelectDelegate {
...
extension LocationController: ClusterManagerDelegate {
