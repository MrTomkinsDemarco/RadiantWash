...
class MainController: UIViewController {
  ...
  private func checkProgressStatus() {
    ...
    if diskSpaceForStartingScreen[.userPhoto] == nil {
      self.setupProgressSize(for: .userPhoto, progress: 0)
    }
    
    if diskSpaceForStartingScreen[.userVideo] == nil {
      self.setupProgressSize(for: .userVideo, progress: 0)
    }
  }
  ...
  private func setupUpdateSizeValues(from type: SingleContentSearchNotificationType, userInfo: [AnyHashable: Any]?) {
    DispatchQueue.main.async {
      ...
      case .photosSizeCheckerType:
        self.setupProgressSize(for: .userPhoto, progress: progress)
      case .videosSizeCheckerType:
        self.setupProgressSize(for: .userVideo, progress: progress)
      default:
        return
      }
    }
  ...
  private func setupProgressSize(for type: MediaContentType, progress: CGFloat) {
    U.UI {
  ...
  private func openMediaController(type: MediaContentType, animated: Bool = true, cleanType: RemoteCleanType) {
    ...
    let viewController = storyboard.instantiateViewController(withIdentifier: C.identifiers.viewControllers.content) as! MediaContentController
  ...
  private func openDeepCleanController(animated: Bool = true) {
    ...
    let viewController = storyboard.instantiateViewController(withIdentifier: C.identifiers.viewControllers.deepClean) as! DeepCleaningController
  ...
  private func openSettingsController() {
    coordinator?.showSettingsController(from: self.navigationController)
  }
  ...
  private func popTopMainController() {
...
extension MainController: UpdateContentDataBaseListener {
...
extension MainController: UpdateMediaStoreSizeDelegate {
...
extension MainController: RemoteLaunchServiceListener {
...
extension MainController: BottomActionButtonDelegate, StartingNavigationBarDelegate {
...
extension MainController: SubscriptionObserver {
...
extension MainController: AppearanceDelegate {
