//
//  VideoCompressingController.swift
//  RadiantWash
//
...
class VideoCompressingController: UIViewController {
  ...
  private var compressionSettingsViewModel: CompressingViewModel!
  private var compressionSettingsDataSource: CompressingDataSource!
...
extension VideoCompressingController {
...
extension VideoCompressingController: ProgressAlertControllerDelegate {
...
extension VideoCompressingController: NavigationBarDelegate {
...
extension VideoCompressingController {
  ...
  private func setupViewModel() {
    ...
    self.compressionSettingsViewModel = CompressingViewModel(sections: sections, phasset: phasset)
    self.compressionSettingsDataSource = CompressingDataSource(compressionSettinsViewModel: self.compressionSettingsViewModel)
...
extension VideoCompressingController: CompressionSettingsActionsDelegate {
  ...
  func setCompressionSettingsForValue(with model: ComprssionModel) {
    ...
    switch model {
    case .custom(fps: _, bitrate: _, scale: _):
      self.openCustomSettingsController()
    default:
      self.currentCompressionModel = model
    }
  }
  
  private func openCustomSettingsController() {
...
extension VideoCompressingController {
  ...
  private func setupShowCustomCompressionSettingsController(segue: UIStoryboardSegue) {
    ...
    guard let segue = segue as? ShowVideoCompressionCustomSettingsSelectorControllerSegue else { return }
    ...
    if let customSettingViewController = segue.destination as? VideoCompressionController {
