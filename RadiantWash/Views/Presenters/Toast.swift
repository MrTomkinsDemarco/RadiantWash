...
public extension UIView {
    ...
    func makeToastInPoint(_ message: String?, duration: TimeInterval = ToastManager.shared.duration, position: CGPoint, title: String? = nil, image: UIImage? = nil, style: ToastStyle = ToastManager.shared.style, completion: ((_ didTap: Bool) -> Void)? = nil) {
        
        do {
            let toast = try toastViewForMessage(message, title: title, image: image, style: style)
            showToast(toast, duration: duration, point: position, completion: completion)
        } catch ToastError.missingParameters {
            print("Error: message, title, and image are all nil")
        } catch {}
     }
    ...
    func showToastInPosition(_ toast: UIView, duration: TimeInterval = ToastManager.shared.duration, position: CGPoint, completion: ((_ didTap: Bool) -> Void)? = nil) {
        showToast(toast, duration: duration, point: position, completion: completion)
    }
...
public enum ToastPosition {
    ...
    /// custom point view in postiotion
    fileprivate func customPoint(_ customPoint: CGPoint) -> CGPoint {
        return CGPoint(x: customPoint.x, y: customPoint.y)
    }
}
