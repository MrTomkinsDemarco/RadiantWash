...
class DeepCleanInfoCell: UITableViewCell {
  ...
  func setupProgress(files count: Int) {
    
    infoTotalFilesTextLabel.text = String(count)
  }
  
  func setupMemmoryChecker(bytes value: Int64) {
  ...
  func setupRoundedProgress(value: CGFloat) {
    circleProgressView.setProgress(progress: value / 100, animated: false)
  }
}

extension DeepCleanInfoCell: Themeble {
  ...
  func setupAppearance() {
    ...
    circleProgressView.backgroundShapeColor = theme.bottomShadowColor.withAlphaComponent(0.2)
    ...
    circleProgressView.backgroundShadowColor = .clear
