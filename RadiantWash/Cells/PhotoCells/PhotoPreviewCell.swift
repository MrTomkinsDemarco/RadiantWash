...
class PhotoPreviewCell: UICollectionViewCell {
  ...
  @IBOutlet weak var playCurrentItemButton: UIButton!
  ...
  public func setupPreview(from phasset: PHAsset, imageManager: PHCachingImageManager, size: CGSize, of contentType: MediaContentType) {
  ...
  public func setupLayout(with contentType: MediaContentType) {
    
    switch contentType {
    case .userPhoto:
      setupVideoLayout(isActive: false)
    case .userVideo:
      setupVideoLayout(isActive: true)
    default:
      return
    }
  }
  
  private func setupVideoLayout(isActive: Bool) {
  ...
  private func setupPHAssetPlayerObserver(for playerItem: AVPlayerItem?) {
  ...
  private func setupAnimateLagePlayButton() {
  ...
  @objc func handlePlayPauseCenterVideoPreviewTap() {
    self.didPlayCurrenMediaItem { successfully in
      successfully ? self.setupAnimateLagePlayButton() : ()
    }
  }
}

extension PhotoPreviewCell: Themeble {
